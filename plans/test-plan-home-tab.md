...
> Format Painter reads `isBold`, `isItalic`, `isUnderline`, `headPrefix` from source. It applies in order: underline → italic → bold → heading prefix.
...
### 14.6 Subscript (`/<sub>/` on raw `line`)

| # | Cursor line text | Subscript active? |
|---|-----------------|-------------------|
| 1 | `x<sub>2</sub>` | YES |
| 2 | `x2` | NO |
| 3 | `<sub>` alone | YES |
| 4 | `<SUB>` | NO (case sensitive) |

### 14.7 Superscript (`/<sup>/` on raw `line`)

| # | Cursor line text | Superscript active? |
|---|-----------------|---------------------|
| 1 | `x<sup>2</sup>` | YES |
| 2 | `x2` | NO |
| 3 | `<sup>` alone | YES |
...
| 2 | `*hello*` | "*hello*" | Italic | `hello` |
| 3 | `*hello*` | "hello" (inner) | Italic | `*hello*` (wraps again → `**hello**` — now looks like bold!) |
| 4 | `***hello***` | "***hello***" | Italic | sel starts with `*`, ends with `*` → `**hello**` (one layer stripped) |
...
This section exhaustively covers every dropdown/menu in the Home tab. For each, the question is: *what happens if you pick the same item twice, or switch between items?* It also clarifies which controls have **no dropdown** despite showing a `▾` caret.
...
| *(all others)* | Insert callout notation | Toggle off when cursor on header line |
