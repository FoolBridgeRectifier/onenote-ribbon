...
export function StylesGroup({ editorState, stylesOffset, setStylesOffset }: Props) {
  ...
  const applyStyle = (index: number) => {
    ...
    const styleEntry = STYLES_LIST[index]; if (!styleEntry) return;
    const cursor  = editor.getCursor();
    const line    = editor.getLine(cursor.line);
    const stripped = line.replace(/^#{1,6}\s+/, '');
    if (styleEntry.prefix) {
      if (line === styleEntry.prefix + stripped) editor.setLine(cursor.line, stripped);
  ...
  const isActiveStyle = (index: number) => {
    const styleEntry = STYLES_LIST[index]; if (!styleEntry) return false;
    const { headLevel } = editorState;
    return (headLevel > 0 && styleEntry.prefix === '#'.repeat(headLevel) + ' ')
  ...
  const clearFormattingItem = {
    ...
    action: () => {
      ...
      const rawText = hasSelection ? editor.getSelection() : editor.getLine(editor.getCursor().line);
  ...
  const dropItems = [
    ...STYLES_LIST.map((styleEntry, index) => ({
      label: styleEntry.label, style: styleEntry.style + ';padding:4px 12px',
  ...
  return (
    <GroupShell name="Styles" dataGroup="Styles">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, width: 130 }}>
          {[0, 1].map((rowIndex) => {
            ...
            return styleEntry ? (
              ...
              >
                <span style={parseCssString(styleEntry.style)}>{styleEntry.label}</span>
              </div>
            ) : null;
          })}
        </div>
      ...
      {dropAnchor && (
        <Dropdown
          anchor={dropAnchor} items={dropItems}
