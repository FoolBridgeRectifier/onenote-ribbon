...
export function ClipboardGroup() {
  ...
  const [pasteMenuAnchor, setPasteMenuAnchor] = useState<HTMLElement | null>(
    null,
  );
  ...
  const paste = () =>
    navigator.clipboard
      .readText()
      .then((clipboardText) => getEditor()?.replaceSelection(clipboardText));
  ...
  const cut = () => {
    ...
    if (selection) {
      navigator.clipboard
        .writeText(selection)
        .then(() => getEditor()?.replaceSelection(""));
    }
  };
