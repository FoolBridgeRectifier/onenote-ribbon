...
export function clearFormatting(
  input: string | { getSelection(): string; getCursor(): { line: number; ch: number }; getLine(n: number): string; replaceSelection(text: string): void; setLine(n: number, text: string): void },
  optionsOrPreserve?: ClearFormattingOptions | boolean,
): string | void {
  if (typeof input === 'string') {
    const opts = typeof optionsOrPreserve === 'boolean' ? { preserveHeadings: optionsOrPreserve } : (optionsOrPreserve ?? {});
