...
describe("BlocksGroup — integration", () => {
  it.skip("renders Template, Callout, and Code Block buttons", () => {
  ...
  it.skip("Template calls executeCommandById('insert-template')", () => {
    const { app } = createAppWithEditor("");
    renderWithApp(<BlocksGroup />, app);
  ...
  it.skip("Code Block is no-op when no active editor", () => {
    const app = createMockApp();
    renderWithApp(<BlocksGroup />, app);
    expect(() => fireEvent.click(screen.getByText("Code Block"))).not.toThrow();
  });

  it.skip("Callout button opens dropdown with callout types", () => {
    const { app } = createAppWithEditor("");
    renderWithApp(<BlocksGroup />, app);
  ...
  it.skip("Callout dropdown item inserts callout markdown", () => {
    const { app, editor } = createAppWithEditor("");
    editor.setCursor({ line: 0, ch: 0 });
    renderWithApp(<BlocksGroup />, app);
    fireEvent.click(screen.getByText("Callout"));
  ...
  it.skip("Template is no-op when no active editor (no crash)", () => {
    const app = createMockApp();
    renderWithApp(<BlocksGroup />, app);
...
describe("BlocksGroup — Callout all 12 types", () => {
  ...
  for (const type of CALLOUT_TYPES) {
    it.skip(`Callout "${type}" inserts > [!${type}] markdown`, () => {
      const { app, editor } = createAppWithEditor("");
      editor.setCursor({ line: 0, ch: 0 });
      renderWithApp(<BlocksGroup />, app);
      fireEvent.click(screen.getByText("Callout"));
