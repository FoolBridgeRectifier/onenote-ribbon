...
describe("ImagesGroup — integration", () => {
  it.skip("renders Image and Video buttons", () => {
  ...
  it.skip("Image inserts ![[]] at cursor", () => {
  ...
  it.skip("Video inserts iframe template at cursor", () => {
    const { app, editor } = createAppWithEditor("");
    editor.setCursor({ line: 0, ch: 0 });
    renderWithApp(<ImagesGroup />, app);
  ...
  it.skip("Image is no-op when no active editor", () => {
    const app = createMockApp();
    renderWithApp(<ImagesGroup />, app);
    expect(() => fireEvent.click(screen.getByText("Image"))).not.toThrow();
  });

  it.skip("Video is no-op when no active editor", () => {
    const app = createMockApp();
    renderWithApp(<ImagesGroup />, app);
