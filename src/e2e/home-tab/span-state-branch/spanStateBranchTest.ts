...
export async function spanStateBranchTest(): Promise<SuiteTestResult[]> {
  return runHomeTabSuite('span-state-branch', async ({ editor, wait, clickByCommand }) => {
    ...
    const fontFamilyContent = '<span style="font-family:Arial;">text with font family</span>';
    editor.setValue(fontFamilyContent);
    ...
    // Move cursor inside bold — bold button must activate (proves state derivation ran with cursor pos)
