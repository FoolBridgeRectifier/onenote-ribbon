 ...
 */
export const propagateCalloutTitles = (content: string, matches: TMatchRecord[]): TMatchRecord[] => {
  ...
  for (const match of matches) {
    ...
    const titleChain = titleStack.slice(0, depth).filter((entry): entry is string => entry !== undefined);
