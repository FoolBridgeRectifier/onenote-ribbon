...
export function useEditorState(app: App): EditorState {
  ...
  useEffect(() => {
    const read = () => {
      ...
      const isInsideTag = (openTag: string, closeTag: string) => {
        ...
        while (position < line.length) {
          ...
          if (
            charPosition > openIndex + openTag.length - 1 &&
            charPosition < closeIndex + closeTag.length
          )
            return true;
    ...
    const leafChangeEventRef = app.workspace.on(
      "active-leaf-change",
      onLeafChange,
    );
    const editorChangeEventRef = app.workspace.on(
      "editor-change",
      onEditorChange,
    );
