...
describe("useEditorState — §14.1 Bold detection", () => {
  it.skip("1 `**hello**` → bold: true", () =>
    expect(readState("**hello**").bold).toBe(true));
  it.skip("2 `hello` → bold: false", () =>
    expect(readState("hello").bold).toBe(false));
  it.skip("3 `**hello** world` → bold: true (partial bold on line)", () =>
    expect(readState("**hello** world").bold).toBe(true));
  it.skip("4 `world **hello**` → bold: true (bold anywhere on line)", () =>
    expect(readState("world **hello**").bold).toBe(true));
  it.skip("5 `*hello*` → bold: false (single-star italic, not bold)", () =>
    expect(readState("*hello*").bold).toBe(false));
  it.skip("6 `***hello***` → bold: true (triple-star contains ** markers)", () =>
    expect(readState("***hello***").bold).toBe(true));
  it.skip("7 `**` (just two stars) → bold: false (needs two separate pairs)", () =>
    expect(readState("**").bold).toBe(false));
  it.skip("8 `<span style='font-weight:bold'>**hi**</span>` → bold: true (HTML stripped first)", () =>
    expect(readState('<span style="font-weight:bold">**hi**</span>').bold).toBe(
      true,
    ));
  it.skip("9 `~~**bold+strike**~~` → bold: true (bold inside strikethrough)", () =>
    expect(readState("~~**bold+strike**~~").bold).toBe(true));
  it.skip("10 `## **heading bold**` → bold: true (heading prefix doesn't interfere)", () =>
...
describe("useEditorState — §14.2 Italic detection", () => {
  it.skip("1 `*hello*` → italic: true", () =>
    expect(readState("*hello*").italic).toBe(true));
  it.skip("2 `hello` → italic: false", () =>
    expect(readState("hello").italic).toBe(false));
  it.skip("3 `**hello**` → italic: false (lookbehind prevents match)", () =>
    expect(readState("**hello**").italic).toBe(false));
  it.skip("4 `***hello***` → italic: true (triple-star also triggers italic regex)", () =>
    expect(readState("***hello***").italic).toBe(true));
  it.skip("5 `*hello* world` → italic: true", () =>
    expect(readState("*hello* world").italic).toBe(true));
  it.skip("6 `world *hello*` → italic: true (italic anywhere on line)", () =>
    expect(readState("world *hello*").italic).toBe(true));
  it.skip("7 `_italic_` → italic: false (underscore syntax not detected)", () =>
    expect(readState("_italic_").italic).toBe(false));
  it.skip("8 `**bold** *italic*` → italic: true (italic anywhere on line)", () =>
    expect(readState("**bold** *italic*").italic).toBe(true));
  it.skip("9 `*a*b*c*` → italic: true (matches first *a*)", () =>
    expect(readState("*a*b*c*").italic).toBe(true));
  it.skip("10 `## *italic heading*` → italic: true", () =>
    expect(readState("## *italic heading*").italic).toBe(true));
  it.skip("bold is false when only italic present", () =>
    expect(readState("*hello*").bold).toBe(false));
});
...
describe("useEditorState — §14.3 Underline detection", () => {
  it.skip("1 `<u>hello</u>` → underline: true", () =>
    expect(readState("<u>hello</u>").underline).toBe(true));
  it.skip("2 `hello` → underline: false", () =>
    expect(readState("hello").underline).toBe(false));
  it.skip("3 `<u>` (no close) → underline: true (open tag alone matches)", () =>
    expect(readState("<u>").underline).toBe(true));
  it.skip("4 `**<u>bold under</u>**` → underline: true", () =>
    expect(readState("**<u>bold under</u>**").underline).toBe(true));
  it.skip("5 `&lt;u&gt;hello&lt;/u&gt;` → underline: false (escaped, not raw HTML)", () =>
    expect(readState("&lt;u&gt;hello&lt;/u&gt;").underline).toBe(false));
  it.skip("6 `<U>hello</U>` → underline: false (regex is case-sensitive)", () =>
    expect(readState("<U>hello</U>").underline).toBe(false));
});
...
describe("useEditorState — §14.4 Strikethrough detection", () => {
  it.skip("1 `~~hello~~` → strikethrough: true", () =>
    expect(readState("~~hello~~").strikethrough).toBe(true));
  it.skip("2 `hello` → strikethrough: false", () =>
    expect(readState("hello").strikethrough).toBe(false));
  it.skip("3 `~~hello~~ world` → strikethrough: true", () =>
    expect(readState("~~hello~~ world").strikethrough).toBe(true));
  it.skip("4 `**~~bold+strike~~**` → strikethrough: true", () =>
    expect(readState("**~~bold+strike~~**").strikethrough).toBe(true));
  it.skip("5 `~hello~` → strikethrough: false (single tilde not matched)", () =>
    expect(readState("~hello~").strikethrough).toBe(false));
  it.skip("6 `~~~~` (empty) → strikethrough: true ((.*?) matches empty string)", () =>
...
describe("useEditorState — §14.5 Highlight detection", () => {
  it.skip("1 `==hello==` → highlight: true", () =>
    expect(readState("==hello==").highlight).toBe(true));
  it.skip("2 `hello` → highlight: false", () =>
    expect(readState("hello").highlight).toBe(false));
  it.skip("3 `==hello== world` → highlight: true", () =>
    expect(readState("==hello== world").highlight).toBe(true));
  it.skip("4 `**==bold+highlight==**` → highlight: true", () =>
    expect(readState("**==bold+highlight==**").highlight).toBe(true));
  it.skip("5 `=hello=` → highlight: false (single = not matched)", () =>
    expect(readState("=hello=").highlight).toBe(false));
  it.skip("6 `====` (empty) → highlight: true (empty match)", () =>
...
describe("useEditorState — §14.6 Subscript cursor-aware detection", () => {
  it.skip("1 ch=0 (before opening tag) → subscript: false", () =>
    expect(readState(SUB_LINE, 0).subscript).toBe(false));
  it.skip("2 ch=1 (at `<` of <sub>) → subscript: false", () =>
    expect(readState(SUB_LINE, 1).subscript).toBe(false));
  it.skip("3 ch=5 (at `>` of <sub> — still inside opening tag) → subscript: false", () =>
    expect(readState(SUB_LINE, 5).subscript).toBe(false));
  it.skip("4 ch=6 (first char of content) → subscript: true", () =>
    expect(readState(SUB_LINE, 6).subscript).toBe(true));
  it.skip("5 ch=7 (inside content) → subscript: true", () =>
    expect(readState(SUB_LINE, 7).subscript).toBe(true));
  it.skip("6 ch=12 (last char of </sub>) → subscript: true", () =>
    expect(readState(SUB_LINE, 12).subscript).toBe(true));
  it.skip("7 ch=13 (past closing tag) → subscript: false", () =>
    expect(readState(SUB_LINE, 13).subscript).toBe(false));
  it.skip("8 `x2` with any ch → subscript: false (no tag present)", () =>
    expect(readState("x2", 1).subscript).toBe(false));
  it.skip("9 multiple spans: cursor inside first (ch=5, content position) → subscript: true", () =>
    expect(readState("<sub>2</sub><sub>3</sub>", 5).subscript).toBe(true));
  it.skip("10 multiple spans: cursor between spans (ch=12, start of second <sub>) → subscript: false", () =>
    expect(readState("<sub>2</sub><sub>3</sub>", 12).subscript).toBe(false));
  it.skip("11 `<SUB>2</SUB>` inside → subscript: false (case-sensitive)", () =>
...
describe("useEditorState — §14.7 Superscript cursor-aware detection", () => {
  it.skip("1 ch=0 (before tag) → superscript: false", () =>
    expect(readState(SUP_LINE, 0).superscript).toBe(false));
  it.skip("2 ch=5 (at `>` of <sup>) → superscript: false", () =>
    expect(readState(SUP_LINE, 5).superscript).toBe(false));
  it.skip("3 ch=6 (content start) → superscript: true", () =>
    expect(readState(SUP_LINE, 6).superscript).toBe(true));
  it.skip("4 ch=12 (last char of </sup>) → superscript: true", () =>
    expect(readState(SUP_LINE, 12).superscript).toBe(true));
  it.skip("5 ch=13 (past closing tag) → superscript: false", () =>
    expect(readState(SUP_LINE, 13).superscript).toBe(false));
  it.skip("6 `x2` → superscript: false (no tag)", () =>
    expect(readState("x2", 1).superscript).toBe(false));
  it.skip("7 `<SUP>` inside → superscript: false (case-sensitive)", () =>
...
describe("useEditorState — §14.8 Bullet List detection", () => {
  it.skip("1 `- item` → bulletList: true", () =>
    expect(readState("- item").bulletList).toBe(true));
  it.skip("2 `item` → bulletList: false", () =>
    expect(readState("item").bulletList).toBe(false));
  it.skip("3 `  - indented` → bulletList: true (leading spaces allowed)", () =>
    expect(readState("  - indented").bulletList).toBe(true));
  it.skip("4 `- [ ] checkbox` → bulletList: true (starts with `- `)", () =>
    expect(readState("- [ ] checkbox").bulletList).toBe(true));
  it.skip("5 `- [x] done` → bulletList: true", () =>
    expect(readState("- [x] done").bulletList).toBe(true));
  it.skip("6 `1. numbered` → bulletList: false", () =>
    expect(readState("1. numbered").bulletList).toBe(false));
  it.skip("7 `-item` (no space) → bulletList: false (requires `- ` with space)", () =>
    expect(readState("-item").bulletList).toBe(false));
  it.skip("8 `## - heading` → bulletList: false (`^` anchors to line start; `#` comes first)", () =>
...
describe("useEditorState — §14.9 Numbered List detection", () => {
  it.skip("1 `1. item` → numberedList: true", () =>
    expect(readState("1. item").numberedList).toBe(true));
  it.skip("2 `42. item` → numberedList: true (any digit count)", () =>
    expect(readState("42. item").numberedList).toBe(true));
  it.skip("3 `  1. indented` → numberedList: true (leading spaces)", () =>
    expect(readState("  1. indented").numberedList).toBe(true));
  it.skip("4 `- item` → numberedList: false", () =>
    expect(readState("- item").numberedList).toBe(false));
  it.skip("5 `1.item` (no space) → numberedList: false", () =>
    expect(readState("1.item").numberedList).toBe(false));
  it.skip("6 `## 1. heading` → numberedList: false (`^` anchors; `#` comes first)", () =>
...
describe("useEditorState — §14.10 Heading level detection", () => {
  it.skip("1 `# Heading` → headLevel: 1", () =>
    expect(readState("# Heading").headLevel).toBe(1));
  it.skip("2 `## Heading` → headLevel: 2", () =>
    expect(readState("## Heading").headLevel).toBe(2));
  it.skip("3 `### Heading` → headLevel: 3", () =>
    expect(readState("### Heading").headLevel).toBe(3));
  it.skip("4 `#### Heading` → headLevel: 4", () =>
    expect(readState("#### Heading").headLevel).toBe(4));
  it.skip("5 `##### Heading` → headLevel: 5", () =>
    expect(readState("##### Heading").headLevel).toBe(5));
  it.skip("6 `###### Heading` → headLevel: 6", () =>
    expect(readState("###### Heading").headLevel).toBe(6));
  it.skip("7 `Plain text` → headLevel: 0", () =>
    expect(readState("Plain text").headLevel).toBe(0));
  it.skip("8 `- item` → headLevel: 0", () =>
    expect(readState("- item").headLevel).toBe(0));
  it.skip("9 `#not-heading` (no space after #) → headLevel: 0", () =>
...
describe("useEditorState — §14.11 Combination detections", () => {
  ...
  it.skip("14 `- <u>**bold under list**</u>` → bulletList + bold + underline", () => {
    const s = readState("- <u>**bold under list**</u>");
    expect(s.bulletList).toBe(true);
    expect(s.bold).toBe(true);
    expect(s.underline).toBe(true);
  });
  it.skip("15 `1. ~~**bold strike numbered**~~` → numberedList + bold + strikethrough", () => {
...
describe("useEditorState — §14.12 False-positive / false-negative edge cases", () => {
  it.skip("1 `**` (just two stars) → bold: false (no closing pair)", () =>
    expect(readState("**").bold).toBe(false));
  it.skip("2 `* item` (unordered list with star) → italic: false (no closing *)", () =>
    expect(readState("* item").italic).toBe(false));
  it.skip("3 `_italic_` → italic: false (underscore syntax not detected)", () =>
    expect(readState("_italic_").italic).toBe(false));
  it.skip("4 `***text***` → bold: true AND italic: true (both match)", () => {
    const s = readState("***text***");
    expect(s.bold).toBe(true);
    expect(s.italic).toBe(true);
  });
  it.skip("5 `==` (just two equals, no close) → highlight: false", () =>
    expect(readState("==").highlight).toBe(false));
  it.skip("6 `<u>text` (no closing tag) → underline: true (only checks open tag)", () =>
    expect(readState("<u>text").underline).toBe(true));
  it.skip("7 `~~` (just two tildes) → strikethrough: false", () =>
    expect(readState("~~").strikethrough).toBe(false));
  it.skip("8 ` - item` (leading space) → bulletList: true (^(\\s*)- allows whitespace)", () =>
    expect(readState(" - item").bulletList).toBe(true));
  it.skip("9 `text - note` (dash mid-line) → bulletList: false (^ anchors to start)", () =>
    expect(readState("text - note").bulletList).toBe(false));
  it.skip("10 `100. item` → numberedList: true (\\d+ matches multiple digits)", () =>
...
describe("useEditorState — no active editor returns all-false state", () => {
  it.skip("returns DEFAULT state when no editor is open", () => {
